...
# Initialize cache loader for performance optimization
@st.cache_resource
...
def train_advanced_model(df, symbol_name, ai_model_choice="AttentionLSTM (Recommended)", progress_callback=None):
    ...
    try:
        # Check if we have cached forecasts for ultra-fast predictions
        if cache_loader.is_cache_available(symbol_name):
            st.info("🚀 Using cached model for lightning-fast predictions!")
            cached_forecasts = cache_loader.get_cached_forecasts(symbol_name, 1825)  # 5 years
            if cached_forecasts and cache_loader.is_cache_fresh(symbol_name, 24):
                st.success(f"✅ Loaded cached 5-year forecasts for {symbol_name}")
                # Convert cached forecasts to expected format
                predictions = np.array(cached_forecasts['predictions'])
                return None, None, predictions  # Return cached predictions
            else:
                st.info("🔄 Cache available but not fresh, using cached model for live prediction...")
                # Use cached model for live prediction
                cached_predictions = cache_loader.fast_predict(symbol_name, 1825)
                if cached_predictions:
                    st.success(f"✅ Generated fresh predictions using cached model for {symbol_name}")
                    return None, None, np.array(cached_predictions)
        
        # Fall back to original training if no cache available
        st.info("🔄 Training new model (no cache available)...")
...
def main():
    ...
    with st.sidebar:
        ...
        # Asset Selection with cache indicators
    ...
    if execute_analysis:
        ...
        # Check if long-term scenario analysis is selected OR if cache is available for faster processing
        ...
        if target_mode == "⚡ Ultra-Fast Cache Mode (Recommended)" or cache_available:
            if cache_available:
                ...
                # Create a simple progress bar for visual feedback
                progress_bar = st.progress(0)
                status_text = st.empty()
            ...
            # For advanced modes, try cache first, then fall back to live training
            if cache_available:
                st.markdown("## ⚡ ADVANCED ANALYSIS (CACHED)")
                
                # Use cached data for advanced analysis
                progress_bar = st.progress(0)
                status_text = st.empty()
                ...
                if cached_forecasts:
                    ...
                    # Fall back to live training for advanced modes
                ...
                # No cache available for advanced modes
                if target_mode == "🎯 Advanced AttentionLSTM + Market Analysis":
                    st.markdown("## 🎯 ADVANCED ATTENTION-LSTM MARKET ANALYSIS")
                    analysis = analyze_long_term_scenarios(symbol, confidence_level=confidence_level)
                    if analysis:
        ...
        # If we have predictions from cache, skip to results display
        if 'predictions' in locals() and predictions is not None:
            pass  # Continue to results display below
        else:
            # Traditional LSTM Analysis for cases where cache is not available or not selected
            ...
            with progress_container:
                ...
                # Step 3: Check for cached forecasts first (ULTRA-FAST MODE)
                update_progress(60, "🔍 CHECKING CACHE AVAILABILITY...")
                
                predictions = None
                if cache_loader.is_cache_available(symbol):
                    update_progress(70, "⚡ LOADING CACHED FORECASTS...")
                    cached_forecasts = cache_loader.get_cached_forecasts(symbol, days)
                    if cached_forecasts:
                        predictions = cached_forecasts['predictions'][:days]  # Get exactly what we need
                        update_progress(100, "🚀 ULTRA-FAST CACHE PREDICTIONS LOADED!")
                        time.sleep(0.5)
                        
                        # Show cache performance message
                        progress_bar.empty()
                        status_text.empty()
                        st.success("⚡ **ULTRA-FAST MODE**: Loaded predictions from cache in milliseconds!")
                        st.info(f"📊 Using pre-computed {len(predictions)}-day forecast generated on {cached_forecasts.get('generated_date', 'Unknown date')}")
                    else:
                        update_progress(75, "🔄 CACHE MISS - FALLING BACK TO LIVE TRAINING...")
                
                # If no cached predictions, fall back to live training
                if predictions is None:
                    update_progress(80, "🧠 INITIALIZING NEURAL NETWORKS...")
        ...
        if 'predictions' not in locals() or predictions is None or len(predictions) == 0:
